	...
	*/	
	function actualizarSaldo($param){
		...
		*/
		...
		// Esto es ejecutado autom�ticamente al finalizar el script.
	}
	...
	*/
	...
	function procesarCobroMensual(){
		...
		if(($nro = mysql_num_rows($result)) == 0){
			...
			echo "<h5>Encontrados ".$nro." clientes sin debito mensual por servicio para el mes y a�o en curso, creando movimiento DMS del mes actual.</h5>";
...
*/
  ...
  <tr class="FondoTituloCol">
    ...
    <td><h5>MAC 2</h5></td>
		...
		<?php 
		$SERVIDORES = array('QUINIMARI' => 1,'PUEBLONUEVO' => 2,'ESMERALDINA' => 3);
		//$STR_IPsCOL = array('IP','IP2','IP3','IP4');
		
		while ($row = mysql_fetch_assoc($result)) {
			$STR_IPsCOL = array($row['ip'],$row['ip2'],$row['ip3'],$row['ip4']);
			foreach ($STR_IPsCOL as $ipcol) {
				//echo " [".$ipcol."]<br>";
				if($ipcol != ''){
					if($row['servidor'] == $SERVIDORES['QUINIMARI'] || $row['servidor'] == $SERVIDORES['PUEBLONUEVO']  ){
						$sIpline[$SERVIDORES['QUINIMARI']] 		.= " ".str_replace("144.0.","172.16.",$ipcol);
						$sIpline[$SERVIDORES['PUEBLONUEVO']] 	.= " ".str_replace("172.16.","144.0.",$ipcol);						
					}
					if($row['servidor'] == $SERVIDORES['ESMERALDINA']){
						$sIpline[$SERVIDORES['ESMERALDINA']] 	.= " ".$ipcol;
					}
				}
			}
			?>
    ...
    <td colspan="12">&nbsp;Numero de Clientes al Dia: <?php echo $total; ?> -COMPAZ COMUNICACIONES 2012-</td>
		...
		$iplist->servidor[0]->address = $sIpline[$SERVIDORES['QUINIMARI']];//quinimari
		$iplist->servidor[1]->address = $sIpline[$SERVIDORES['PUEBLONUEVO']];//pueblonuevo
		$iplist->servidor[2]->address = $sIpline[$SERVIDORES['ESMERALDINA']];//esmeraldina
